...
void CFE_TBL_GetHkData(void)
{
    ...
    if ((CFE_TBL_TaskData.LastTblUpdated >= 0) && (CFE_TBL_TaskData.LastTblUpdated < CFE_PLATFORM_TBL_MAX_NUM_TABLES))
    {
        /* Check to make sure the Registry Entry is still valid */
        if (!CFE_ES_ResourceID_Equal(CFE_TBL_TaskData.Registry[CFE_TBL_TaskData.LastTblUpdated].OwnerAppId,
                                     CFE_TBL_NOT_OWNED))
        {
...
void CFE_TBL_GetTblRegData(void)
{
    ...
    CFE_TBL_TaskData.TblRegPacket.Payload.FileCreateTimeSecs =
        RegRecPtr->Buffers[RegRecPtr->ActiveBufferIndex].FileCreateTimeSecs;
    CFE_TBL_TaskData.TblRegPacket.Payload.FileCreateTimeSubSecs =
    ...
    CFE_SB_MessageStringSet(CFE_TBL_TaskData.TblRegPacket.Payload.LastFileLoaded, RegRecPtr->LastFileLoaded,
                            sizeof(CFE_TBL_TaskData.TblRegPacket.Payload.LastFileLoaded),
        ...
        if (Status == sizeof(CFE_FS_Header_t))
        {
            ...
            if (Status == sizeof(CFE_TBL_File_Hdr_t))
            {
                ...
                if (Status == TblSizeInBytes)
                {
                    ...
                    CFE_TBL_TaskData.HkPacket.Payload
                        .LastFileDumped[sizeof(CFE_TBL_TaskData.HkPacket.Payload.LastFileDumped) - 1] = 0;
    ...
    if (RegIndex != CFE_TBL_NOT_FOUND)
    {
        ...
        if (ValidationDataPtr != NULL)
        {
            ...
            if (ValIndex < CFE_PLATFORM_TBL_MAX_NUM_VALIDATIONS)
            {
                ...
                memcpy(CFE_TBL_TaskData.ValidationResults[ValIndex].TableName, TableName, CFE_TBL_MAX_FULL_NAME_LEN);
        ...
        if (Status == sizeof(CFE_FS_Header_t))
        {
            ...
            while ((RegIndex < CFE_PLATFORM_TBL_MAX_NUM_TABLES) && (Status == sizeof(CFE_TBL_RegDumpRec_t)))
            {
                    ...
                    (RegRecPtr->HeadOfAccessList != CFE_TBL_END_OF_LIST))
                {
                    ...
                    DumpRecord.FileCreateTimeSubSecs =
                        RegRecPtr->Buffers[RegRecPtr->ActiveBufferIndex].FileCreateTimeSubSecs;
                    ...
                    if (DumpRecord.LoadInProgress != CFE_TBL_NO_LOAD_IN_PROGRESS)
                    {
                        ...
                        {
                            /* For double buffered tables, the value of LoadInProgress, when a load is actually in
                             * progress, */
                            /* should identify either buffer #0 or buffer #1.  Convert these to enumerated value for
                             * ground  */
                         ...
                         */
                        /* indicates which shared buffer is allocated for the inactive buffer.  Since the number of
                         * inactive */
                        /* buffers is a platform configuration parameter, then 0 on up merely identifies the buffer
                         * number.  */
                    ...
                    strncpy(DumpRecord.LastFileLoaded, RegRecPtr->LastFileLoaded,
                            sizeof(DumpRecord.LastFileLoaded) - 1);
                    ...
                    if (!CFE_ES_ResourceID_Equal(RegRecPtr->OwnerAppId, CFE_TBL_NOT_OWNED))
                    {
                        CFE_ES_GetAppName(DumpRecord.OwnerAppName, RegRecPtr->OwnerAppId,
                                          sizeof(DumpRecord.OwnerAppName));
                    }
                    else
                    {
                    ...
                    Status = OS_write(FileDescriptor, &DumpRecord, sizeof(CFE_TBL_RegDumpRec_t));
            ...
            if (Status == sizeof(CFE_TBL_RegDumpRec_t))
            {
                if (FileExistedPrev)
                {
                    CFE_EVS_SendEvent(CFE_TBL_OVERWRITE_REG_DUMP_INF_EID, CFE_EVS_EventType_DEBUG,
                                      "Successfully overwrote '%s' with Table Registry:Size=%d,Entries=%d",
                                      DumpFilename, (int)FileSize, (int)NumEntries);
                }
                else
                {
                    CFE_EVS_SendEvent(CFE_TBL_WRITE_REG_DUMP_INF_EID, CFE_EVS_EventType_DEBUG,
                                      "Successfully dumped Table Registry to '%s':Size=%d,Entries=%d", DumpFilename,
            ...
            {
                CFE_EVS_SendEvent(CFE_TBL_WRITE_TBL_REG_ERR_EID, CFE_EVS_EventType_ERROR,
                                  "Error writing Registry to '%s', Status=0x%08X", DumpFilename, (unsigned int)Status);
            }
        }
        else
        {
