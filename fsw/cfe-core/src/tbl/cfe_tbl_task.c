...
void CFE_TBL_TaskMain(void)
{
    ...
    while (Status == CFE_SUCCESS)
    {
        ...
        Status = CFE_SB_ReceiveBuffer(&SBBufPtr, CFE_TBL_TaskData.CmdPipe, CFE_SB_PEND_FOREVER);
...
int32 CFE_TBL_TaskInit(void)
{
    ...
    */
    Status = CFE_SB_CreatePipe(&CFE_TBL_TaskData.CmdPipe, CFE_TBL_TaskData.PipeDepth, CFE_TBL_TaskData.PipeName);
    if (Status != CFE_SUCCESS)
    {
