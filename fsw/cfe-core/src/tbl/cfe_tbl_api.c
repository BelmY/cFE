                       ...
                       CFE_TBL_CallbackFuncPtr_t TblValidationFuncPtr)
{
            ...
            *TblHandlePtr = CFE_TBL_FindFreeHandle();
            ...
            if ((Status & CFE_SEVERITY_BITMASK) != CFE_SEVERITY_ERROR)
            {
                ...
                if ((TblOptionFlags & CFE_TBL_OPT_USR_DEF_MSK) != (CFE_TBL_OPT_USR_DEF_ADDR & CFE_TBL_OPT_USR_DEF_MSK))
                {
                    ...
                    Status = CFE_ES_GetPoolBuf(&RegRecPtr->Buffers[0].BufferPtr, CFE_TBL_TaskData.Buf.PoolHdl, Size);
                    if (Status < 0)
                    {
                        CFE_ES_WriteToSysLog(
                    ...
                    ((Status & CFE_SEVERITY_BITMASK) != CFE_SEVERITY_ERROR))
                {
                    ...
                    Status = CFE_ES_GetPoolBuf(&RegRecPtr->Buffers[1].BufferPtr, CFE_TBL_TaskData.Buf.PoolHdl, Size);
                    if (Status < 0)
                    {
                        CFE_ES_WriteToSysLog(
    ...
    if (Status < 0)
    {
        ...
        *TblHandlePtr = CFE_TBL_BAD_TABLE_HANDLE;
        ...
        CFE_EVS_SendEventWithAppID(CFE_TBL_REGISTER_ERR_EID, CFE_EVS_EventType_ERROR, CFE_TBL_TaskData.TableTaskAppId,
...
int32 CFE_TBL_Share(CFE_TBL_Handle_t *TblHandlePtr, const char *TblName)
{
    ...
    if (Status < 0)
    {
        ...
        CFE_EVS_SendEventWithAppID(CFE_TBL_SHARE_ERR_EID, CFE_EVS_EventType_ERROR, CFE_TBL_TaskData.TableTaskAppId,
...
int32 CFE_TBL_Unregister(CFE_TBL_Handle_t TblHandle)
{
    ...
    if (Status < 0)
    {
        ...
        CFE_EVS_SendEventWithAppID(CFE_TBL_UNREGISTER_ERR_EID, CFE_EVS_EventType_ERROR, CFE_TBL_TaskData.TableTaskAppId,
...
int32 CFE_TBL_Load(CFE_TBL_Handle_t TblHandle, CFE_TBL_SrcEnum_t SrcType, const void *SrcDataPtr)
{
    ...
    if (Status != CFE_SUCCESS)
    {
        CFE_EVS_SendEventWithAppID(CFE_TBL_HANDLE_ACCESS_ERR_EID, CFE_EVS_EventType_ERROR,
                                   CFE_TBL_TaskData.TableTaskAppId, "%s: No access to Tbl Handle=%d", AppName,
                                   (int)TblHandle);
    ...
    if (RegRecPtr->DumpOnly)
    {
        if ((!RegRecPtr->UserDefAddr) || (RegRecPtr->TableLoadedOnce))
        {
            CFE_EVS_SendEventWithAppID(CFE_TBL_LOADING_A_DUMP_ONLY_ERR_EID, CFE_EVS_EventType_ERROR,
                                       CFE_TBL_TaskData.TableTaskAppId, "%s: Attempted to load Dump Only Tbl '%s'",
        ...
        CFE_EVS_SendEventWithAppID(CFE_TBL_LOAD_SUCCESS_INF_EID, CFE_EVS_EventType_DEBUG,
                                   CFE_TBL_TaskData.TableTaskAppId, "Successfully loaded '%s' from '%s'",
                                   RegRecPtr->Name, RegRecPtr->Buffers[0].DataSource);
    ...
    if (RegRecPtr->LoadInProgress != CFE_TBL_NO_LOAD_IN_PROGRESS)
    {
        CFE_EVS_SendEventWithAppID(CFE_TBL_LOAD_IN_PROGRESS_ERR_EID, CFE_EVS_EventType_ERROR,
                                   CFE_TBL_TaskData.TableTaskAppId, "%s: Load already in progress for '%s'", AppName,
                                   RegRecPtr->Name);
    ...
    if (Status != CFE_SUCCESS)
    {
        CFE_EVS_SendEventWithAppID(CFE_TBL_NO_WORK_BUFFERS_ERR_EID, CFE_EVS_EventType_ERROR,
                                   CFE_TBL_TaskData.TableTaskAppId, "%s: Failed to get Working Buffer (Stat=%u)",
    ...
    if (!FirstTime)
    {
        ...
        if (Status != CFE_SUCCESS)
        {
            CFE_EVS_SendEventWithAppID(CFE_TBL_UPDATE_ERR_EID, CFE_EVS_EventType_ERROR, CFE_TBL_TaskData.TableTaskAppId,
    ...
    else
    {
        ...
        strncpy(RegRecPtr->LastFileLoaded, WorkingBufferPtr->DataSource, OS_MAX_PATH_LEN);
    ...
    else
    {
        ...
        if (Status == CFE_SUCCESS)
        {
            CFE_EVS_SendEventWithAppID(CFE_TBL_UPDATE_SUCCESS_INF_EID, CFE_EVS_EventType_INFORMATION,
                                       CFE_TBL_TaskData.TableTaskAppId, "%s Successfully Updated '%s'", AppName,
                                       RegRecPtr->Name);
    ...
    if (Status == CFE_SUCCESS)
    {
        ...
        if (RegRecPtr->ValidateInactiveIndex != CFE_TBL_NO_VALIDATION_PENDING)
        {
            ...
            else
            {
                ...
                Status =
                    (RegRecPtr->ValidationFuncPtr)(CFE_TBL_TaskData.LoadBuffs[RegRecPtr->LoadInProgress].BufferPtr);
            ...
            if (Status == CFE_SUCCESS)
            {
                CFE_EVS_SendEventWithAppID(CFE_TBL_VALIDATION_INF_EID, CFE_EVS_EventType_INFORMATION,
                                           CFE_TBL_TaskData.TableTaskAppId,
                                           "%s validation successful for Inactive '%s'", AppName, RegRecPtr->Name);
            }
            else
            {
                CFE_EVS_SendEventWithAppID(CFE_TBL_VALIDATION_ERR_EID, CFE_EVS_EventType_ERROR,
                ...
                if (Status > CFE_SUCCESS)
                {
                    CFE_ES_WriteToSysLog(
                        ...
                        CFE_ES_ResourceID_ToInteger(CFE_TBL_TaskData.TableTaskAppId), (unsigned int)Status,
                        RegRecPtr->Name);
                }
            }
        ...
        else if (RegRecPtr->ValidateActiveIndex != CFE_TBL_NO_VALIDATION_PENDING)
        {
            ...
            {
                CFE_EVS_SendEventWithAppID(CFE_TBL_VALIDATION_INF_EID, CFE_EVS_EventType_INFORMATION,
                                           CFE_TBL_TaskData.TableTaskAppId, "%s validation successful for Active '%s'",
                                           AppName, RegRecPtr->Name);
            }
            else
            {
