...
int32 CFE_TBL_EarlyInit(void)
{
    ...
    */
    Status = OS_MutSemCreate(&CFE_TBL_TaskData.RegistryMutex, CFE_TBL_MUT_REG_NAME, CFE_TBL_MUT_REG_VALUE);
    ...
    */
    Status = OS_MutSemCreate(&CFE_TBL_TaskData.WorkBufMutex, CFE_TBL_MUT_WORK_NAME, CFE_TBL_MUT_WORK_VALUE);
    if (Status != OS_SUCCESS)
    {
        return Status;
    } /* end if */
    ...
    */
    CFE_MSG_Init(&CFE_TBL_TaskData.HkPacket.TlmHeader.Msg, CFE_SB_ValueToMsgId(CFE_TBL_HK_TLM_MID),
                 sizeof(CFE_TBL_TaskData.HkPacket));

    /*
    ** Initialize table registry report packet (clear user data area)...
    */
    CFE_MSG_Init(&CFE_TBL_TaskData.TblRegPacket.TlmHeader.Msg, CFE_SB_ValueToMsgId(CFE_TBL_REG_TLM_MID),
                 sizeof(CFE_TBL_TaskData.TblRegPacket));

    /* Initialize memory partition and allocate shared table buffers. */
    Status = CFE_ES_PoolCreate(&CFE_TBL_TaskData.Buf.PoolHdl, CFE_TBL_TaskData.Buf.Partition.Data,
    ...
    else
    {
        ...
        do
        {
            ...
            Status = CFE_ES_GetPoolBuf(&CFE_TBL_TaskData.LoadBuffs[j].BufferPtr, CFE_TBL_TaskData.Buf.PoolHdl,
                ...
                while ((AccessIterator != CFE_TBL_END_OF_LIST) && (Status == CFE_SUCCESS))
                {
                        ...
                        (CFE_TBL_TaskData.Handles[AccessIterator].LockFlag))
                    {
                        ...
                        CFE_ES_WriteToSysLog(
                            ...
                            RegRecPtr->Name,
                            CFE_ES_ResourceID_ToInteger(CFE_TBL_TaskData.Handles[AccessIterator].AppId));
                    }
                           ...
                           const char *Filename)
{
    ...
    if (FilenameLen > (OS_MAX_PATH_LEN - 1))
    {
        CFE_EVS_SendEventWithAppID(CFE_TBL_LOAD_FILENAME_LONG_ERR_EID, CFE_EVS_EventType_ERROR,
                                   CFE_TBL_TaskData.TableTaskAppId, "%s: Filename is too long ('%s' (%lu) > %lu)",
    ...
    if (Status < 0)
    {
        CFE_EVS_SendEventWithAppID(CFE_TBL_FILE_ACCESS_ERR_EID, CFE_EVS_EventType_ERROR,
                                   CFE_TBL_TaskData.TableTaskAppId, "%s: Unable to open file (FileDescriptor=%d)",
                                   AppName, (int)Status);
    ...
    if (strcmp(RegRecPtr->Name, TblFileHeader.TableName) != 0)
    {
        CFE_EVS_SendEventWithAppID(CFE_TBL_LOAD_TBLNAME_MISMATCH_ERR_EID, CFE_EVS_EventType_ERROR,
                                   CFE_TBL_TaskData.TableTaskAppId, "%s: Table name mismatch (exp=%s, tblfilhdr=%s)",
    ...
    if ((TblFileHeader.Offset + TblFileHeader.NumBytes) > RegRecPtr->Size)
    {
        CFE_EVS_SendEventWithAppID(
            CFE_TBL_LOAD_EXCEEDS_SIZE_ERR_EID, CFE_EVS_EventType_ERROR, CFE_TBL_TaskData.TableTaskAppId,
    ...
    if (NumBytes != TblFileHeader.NumBytes)
    {
        CFE_EVS_SendEventWithAppID(CFE_TBL_FILE_INCOMPLETE_ERR_EID, CFE_EVS_EventType_ERROR,
                                   CFE_TBL_TaskData.TableTaskAppId, "%s: File load incomplete (exp=%lu, read=%lu)",
    ...
    if (NumBytes == 1)
    {
        CFE_EVS_SendEventWithAppID(CFE_TBL_FILE_TOO_BIG_ERR_EID, CFE_EVS_EventType_ERROR,
                                   CFE_TBL_TaskData.TableTaskAppId, "%s: File load too long (file length > %lu)",
            ...
            else
            {
                ...
                if (RegRecPtr->Buffers[0].BufferPtr != CFE_TBL_TaskData.LoadBuffs[RegRecPtr->LoadInProgress].BufferPtr)
                {
                    memcpy(RegRecPtr->Buffers[0].BufferPtr,
                           CFE_TBL_TaskData.LoadBuffs[RegRecPtr->LoadInProgress].BufferPtr, RegRecPtr->Size);
                }
                ...
                strncpy(RegRecPtr->LastFileLoaded, CFE_TBL_TaskData.LoadBuffs[RegRecPtr->LoadInProgress].DataSource,
                          ...
                          CFE_TBL_File_Hdr_t *TblFileHeaderPtr, const char *LoadFilename)
{
    ...
    {
        CFE_EVS_SendEventWithAppID(
            CFE_TBL_FILE_STD_HDR_ERR_EID, CFE_EVS_EventType_ERROR, CFE_TBL_TaskData.TableTaskAppId,
            "Unable to read std header for '%s', Status = 0x%08X", LoadFilename, (unsigned int)Status);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    CFE_EVS_SendEventWithAppID(
                        CFE_TBL_FILE_TBL_HDR_ERR_EID, CFE_EVS_EventType_ERROR, CFE_TBL_TaskData.TableTaskAppId,
...
int32 CFE_TBL_SendNotificationMsg(CFE_TBL_RegistryRec_t *RegRecPtr)
{
    ...
    if (RegRecPtr->NotifyByMsg)
    {
        ...
        */
        CFE_MSG_Init(&CFE_TBL_TaskData.NotifyMsg.CmdHeader.Msg, RegRecPtr->NotificationMsgId,
