...
int32 CFE_ES_ParseFileEntry(const char **TokenList, uint32 NumTokens)
{
    ...
    */
    ...
    if (strcmp(EntryType, "CFE_APP") == 0)
    {
        ...
        */
        CreateStatus =
            CFE_ES_AppCreate(&ApplicationId, FileName, EntryPoint, AppName, PriorityIn, StackSizeIn, ExceptionActionIn);
    }
    else if (strcmp(EntryType, "CFE_LIB") == 0)
    {
        ...
        */
        CreateStatus = CFE_ES_LoadLibrary(&ApplicationId, FileName, EntryPoint, AppName);
    }
    else
    {
                        ...
                        CFE_ES_ModuleLoadStatus_t *LoadStatus)
{
    ...
    if (LoadParams->FileName[0] != 0)
    {
         ...
         */
        StatusCode = OS_ModuleLoad(&ModuleId, LoadParams->Name, LoadParams->FileName, LoadFlags);
...
void CFE_ES_AppEntryPoint(void)
{
    ...
    if (CFE_ES_GetAppEntryPoint(&RealEntryFunc) == CFE_SUCCESS && RealEntryFunc != NULL)
    {
        (*RealEntryFunc)();
    }
}
    ...
    */
    ...
    else
    {
        ...
        else
        {
            ...
            strncpy(AppRecPtr->StartParams.BasicInfo.Name, AppName, sizeof(AppRecPtr->StartParams.BasicInfo.Name) - 1);
    ...
    */
    ...
    else
    {
        ...
        else
        {
             ...
             */
            strncpy(LibSlotPtr->BasicInfo.Name, LibName, sizeof(LibSlotPtr->BasicInfo.Name) - 1);
            strncpy(LibSlotPtr->BasicInfo.FileName, FileName, sizeof(LibSlotPtr->BasicInfo.FileName) - 1);
            if (EntryPointName != NULL && strcmp(EntryPointName, "NULL") != 0)
            {
                strncpy(LibSlotPtr->BasicInfo.EntryPoint, EntryPointName, sizeof(LibSlotPtr->BasicInfo.EntryPoint) - 1);
            }
...
void CFE_ES_ProcessControlRequest(CFE_ES_ResourceID_t AppId)
{
    ...
    {
        StartupStatus =
            CFE_ES_AppCreate(&NewAppId, OrigStartParams.BasicInfo.FileName, OrigStartParams.BasicInfo.EntryPoint,
                             OrigStartParams.BasicInfo.Name, OrigStartParams.Priority, OrigStartParams.StackSize,
    ...
    if (EventID != 0 && ReqName != NULL)
    {
        ...
        CFE_EVS_SendEvent(EventID, EventType, "%s Application %s %s", ReqName, OrigStartParams.BasicInfo.Name,
                          MessageDetail);
    }
...
int32 CFE_ES_CleanUpApp(CFE_ES_ResourceID_t AppId)
{
    ...
    if (CFE_ES_AppRecordIsMatch(AppRecPtr, AppId))
    {
        ...
        {
            if (CFE_ES_TaskRecordIsUsed(TaskRecPtr) && CFE_ES_ResourceID_Equal(TaskRecPtr->AppId, AppId))
            {
                TaskList[NumTasks] = CFE_ES_TaskRecordGetID(TaskRecPtr);

                /* Store the main task ID at index 0 (swap with whatever was there) */
                if (CFE_ES_ResourceID_Equal(TaskList[NumTasks], AppRecPtr->MainTaskId) && NumTasks != 0)
                {
        ...
        {
            if (CFE_ES_MemPoolRecordIsUsed(MemPoolRecPtr) && CFE_ES_ResourceID_Equal(MemPoolRecPtr->OwnerAppID, AppId))
            {
    ...
    if (NumTasks == 0 || !CFE_ES_ResourceID_Equal(TaskList[0], AppRecPtr->MainTaskId))
    {
        CFE_ES_SysLogWrite_Unsync("CFE_ES_CleanUpApp: MainTask ID:%lu not found when deleting AppID %lu\n",
                                  CFE_ES_ResourceID_ToInteger(AppRecPtr->MainTaskId),
                                  CFE_ES_ResourceID_ToInteger(AppId));
        ReturnCode = CFE_ES_APP_CLEANUP_ERR;
    }
#endif
...
{
    strncpy(AppInfoPtr->Name, ParamsPtr->Name, sizeof(AppInfoPtr->Name) - 1);
    AppInfoPtr->Name[sizeof(AppInfoPtr->Name) - 1] = '\0';

    strncpy(AppInfoPtr->EntryPoint, ParamsPtr->EntryPoint, sizeof(AppInfoPtr->EntryPoint) - 1);
