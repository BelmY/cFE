...
*/
int32 CFE_ES_CreateChildTask(CFE_ES_ResourceID_t *TaskIdPtr, const char *TaskName,
        ...
        else /* else AppId is valid */
        {
            ...
            */
            ...
            if (CFE_ES_ResourceID_Equal(TaskId, ParentTaskId))
            {
                ...
                */
                Result = OS_TaskCreate(&OsalId, TaskName, FunctionPtr, StackPtr, StackSize, Priority, OS_FP_ENABLED);
            ...
            {
                CFE_ES_SysLogWrite_Unsync(
                    "CFE_ES_CreateChildTask: Error: Cannot call from a Child Task (for Task '%s').\n", TaskName);
                ReturnCode = CFE_ES_ERR_CHILD_TASK_CREATE;
...
int32 CFE_ES_RegisterGenCounter(CFE_ES_ResourceID_t *CounterIdPtr, const char *CounterName)
{
    ...
    else
    {
        ...
        {
            strncpy(CountRecPtr->CounterName, CounterName, sizeof(CountRecPtr->CounterName));
...
{
    return CFE_ES_ResourceID_ToIndex_Internal(CFE_ES_ResourceID_ToInteger(AppID) - CFE_ES_APPID_BASE,
                                              CFE_PLATFORM_ES_MAX_APPLICATIONS, Idx);
}
...
{
    return CFE_ES_ResourceID_ToIndex_Internal(CFE_ES_ResourceID_ToInteger(LibId) - CFE_ES_LIBID_BASE,
                                              CFE_PLATFORM_ES_MAX_LIBRARIES, Idx);
}
...
{
    return CFE_ES_ResourceID_ToIndex_Internal(CFE_ES_ResourceID_ToInteger(CounterId) - CFE_ES_COUNTID_BASE,
