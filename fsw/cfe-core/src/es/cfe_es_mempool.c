...
{
    return CFE_ES_ResourceID_ToIndex_Internal(CFE_ES_ResourceID_ToInteger(PoolID) - CFE_ES_POOLID_BASE,
                                              CFE_PLATFORM_ES_MAX_MEMORY_POOLS, Idx);
}
                          ...
                          const size_t *BlockSizes, bool UseMutex)
{
    ...
    if (Status == CFE_SUCCESS && UseMutex == CFE_ES_USE_MUTEX)
    {
        ...
        */
        snprintf(MutexName, OS_MAX_API_NAME, "Pool%08lX", CFE_ES_ResourceID_ToInteger(PendingID));
