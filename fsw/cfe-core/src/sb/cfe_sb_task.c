...
void CFE_SB_TaskMain(void)
{
    ...
    while (Status == CFE_SUCCESS)
    {
        ...
        Status = CFE_SB_ReceiveBuffer(&SBBufPtr, CFE_SB.CmdPipe, CFE_SB_PEND_FOREVER);
...
int32 CFE_SB_AppInit(void)
{
    ...
    ** does not exceed CFE_PLATFORM_EVS_MAX_EVENT_FILTERS */
    ...
    Status = CFE_EVS_Register(CFE_SB.EventFilters, CfgFileEventsToFilter, CFE_EVS_EventFilter_BINARY);
    if (Status != CFE_SUCCESS)
    {
    ...
    CFE_MSG_Init(&CFE_SB.HKTlmMsg.Hdr.Msg, CFE_SB_ValueToMsgId(CFE_SB_HK_TLM_MID), sizeof(CFE_SB.HKTlmMsg));

    CFE_MSG_Init(&CFE_SB.PrevSubMsg.Hdr.Msg, CFE_SB_ValueToMsgId(CFE_SB_ALLSUBS_TLM_MID), sizeof(CFE_SB.PrevSubMsg));

    CFE_MSG_Init(&CFE_SB.SubRprtMsg.Hdr.Msg, CFE_SB_ValueToMsgId(CFE_SB_ONESUB_TLM_MID), sizeof(CFE_SB.SubRprtMsg));
    ...
    Status = CFE_SB_CreatePipe(&CFE_SB.CmdPipe, CFE_SB_CMD_PIPE_DEPTH, CFE_SB_CMD_PIPE_NAME);
    if (Status != CFE_SUCCESS)
    {
    ...
    Status = CFE_ES_GetPoolBuf(&TmpPtr, CFE_SB.Mem.PoolHdl, sizeof(CFE_ES_RestartCmd_t));
...
int32 CFE_SB_EnableRouteCmd(const CFE_SB_EnableRouteCmd_t *data)
{
    ...
    if (!CFE_SB_IsValidMsgId(MsgId) || (CFE_SB_ValidatePipeId(PipeId) != CFE_SUCCESS))
    {
        CFE_EVS_SendEvent(CFE_SB_ENBL_RTE3_EID, CFE_EVS_EventType_ERROR,
                          "Enbl Route Cmd:Invalid Param.Msg 0x%x,Pipe %d", (unsigned int)CFE_SB_MsgIdToValue(MsgId),
                          (int)PipeId);
        CFE_SB.HKTlmMsg.Payload.CommandErrorCounter++;
        /*
    ...
    CFE_EVS_SendEvent(CFE_SB_ENBL_RTE2_EID, CFE_EVS_EventType_DEBUG, "Enabling Route,Msg 0x%x,Pipe %d",
                      (unsigned int)CFE_SB_MsgIdToValue(MsgId), (int)PipeId);
...
int32 CFE_SB_DisableRouteCmd(const CFE_SB_DisableRouteCmd_t *data)
{
    ...
    if (!CFE_SB_IsValidMsgId(MsgId) || (CFE_SB_ValidatePipeId(PipeId) != CFE_SUCCESS))
    {
        CFE_EVS_SendEvent(CFE_SB_DSBL_RTE3_EID, CFE_EVS_EventType_ERROR,
                          "Disable Route Cmd:Invalid Param.Msg 0x%x,Pipe %d", (unsigned int)CFE_SB_MsgIdToValue(MsgId),
                          (int)PipeId);
        CFE_SB.HKTlmMsg.Payload.CommandErrorCounter++;
        /*
         * returning "success" here as there is no other recourse;
    ...
    CFE_EVS_SendEvent(CFE_SB_DSBL_RTE2_EID, CFE_EVS_EventType_DEBUG, "Route Disabled,Msg 0x%x,Pipe %d",
                      (unsigned int)CFE_SB_MsgIdToValue(MsgId), (int)PipeId);
    CFE_SB.HKTlmMsg.Payload.CommandCounter++;
...
int32 CFE_SB_SendPipeInfoCmd(const CFE_SB_SendPipeInfoCmd_t *data)
{
    ...
    CFE_SB_MessageStringGet(LocalFilename, ptr->Filename, CFE_PLATFORM_SB_DEFAULT_PIPE_FILENAME, sizeof(LocalFilename),
                            sizeof(ptr->Filename));
...
int32 CFE_SB_SendMapInfoCmd(const CFE_SB_SendMapInfoCmd_t *data)
{
    ...
    CFE_SB_MessageStringGet(LocalFilename, ptr->Filename, CFE_PLATFORM_SB_DEFAULT_MAP_FILENAME, sizeof(LocalFilename),
                            sizeof(ptr->Filename));
...
int32 CFE_SB_SendRtgInfo(const char *Filename)
{
    ...
    {
        CFE_EVS_SendEvent(CFE_SB_SND_RTG_ERR1_EID, CFE_EVS_EventType_ERROR, "Error creating file %s, stat=0x%x",
    ...
    {
        OS_close(args.Fd);
        CFE_EVS_SendEvent(CFE_SB_SND_RTG_EID, CFE_EVS_EventType_DEBUG, "%s written:Size=%d,Entries=%d", Filename,
                          (int)args.FileSize, (int)args.EntryCount);
        return CFE_SUCCESS;
    }
...
int32 CFE_SB_SendPrevSubsCmd(const CFE_SB_SendPrevSubsCmd_t *data)
{
    ...
    if (CFE_SB.PrevSubMsg.Payload.Entries > 0)
    {
        ...
        CFE_EVS_SendEvent(CFE_SB_PART_SUB_PKT_EID, CFE_EVS_EventType_DEBUG,
                          "Partial Sub Pkt %d Sent,Entries=%d,Stat=0x%x", (int)CFE_SB.PrevSubMsg.Payload.PktSegment,
...
void CFE_SB_FileWriteByteCntErr(const char *Filename, uint32 Requested, uint32 Actual)
{
    ...
    CFE_EVS_SendEvent(CFE_SB_FILEWRITE_ERR_EID, CFE_EVS_EventType_ERROR,
                      "File write,byte cnt err,file %s,request=%d,actual=%d", Filename, (int)Requested, (int)Actual);
