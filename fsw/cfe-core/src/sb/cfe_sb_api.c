...
#define CFE_SB_TLM_PIPEDEPTHSTATS_SIZE \
    (sizeof(CFE_SB.StatTlmMsg.Payload.PipeDepthStats) / sizeof(CFE_SB.StatTlmMsg.Payload.PipeDepthStats[0]))
...
int32 CFE_SB_CreatePipe(CFE_SB_PipeId_t *PipeIdPtr, uint16 Depth, const char *PipeName)
{
    ...
    *PipeIdPtr = PipeTblIdx;
    ...
    CFE_EVS_SendEventWithAppID(CFE_SB_PIPE_ADDED_EID, CFE_EVS_EventType_DEBUG, CFE_SB.AppId,
                               "Pipe Created:name %s,id %d,app %s", PipeName, (int)CFE_SB.PipeTbl[PipeTblIdx].PipeId,
                               CFE_SB_GetAppTskName(TskId, FullName));
...
int32 CFE_SB_DeletePipeFull(CFE_SB_PipeId_t PipeId, CFE_ES_ResourceID_t AppId)
{
    ...
    if ((RtnFromVal != CFE_SUCCESS) || (PipeTblIdx == CFE_SB_INVALID_PIPE))
    {
        ...
        CFE_EVS_SendEventWithAppID(CFE_SB_DEL_PIPE_ERR1_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                                   "Pipe Delete Error:Bad Argument,PipedId %d,Requestor %s,Idx %d,Stat %d", (int)PipeId,
                                   FullName, (int)PipeTblIdx, (int)RtnFromVal);
        return CFE_SB_BAD_ARGUMENT;
    } /* end if */
...
int32 CFE_SB_SetPipeOpts(CFE_SB_PipeId_t PipeId, uint8 Opts)
{
    ...
    if ((RtnFromVal != CFE_SUCCESS) || (PipeTblIdx == CFE_SB_INVALID_PIPE))
    {
        ...
        CFE_EVS_SendEventWithAppID(CFE_SB_SETPIPEOPTS_ID_ERR_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                                   "Pipe Opts Error:Bad Argument,PipedId %d,Requestor %s,Idx %d,Stat %d", (int)PipeId,
                                   CFE_SB_GetAppTskName(TskId, FullName), (int)PipeTblIdx, (int)RtnFromVal);
        return CFE_SB_BAD_ARGUMENT;
    } /* end if */
...
int32 CFE_SB_GetPipeOpts(CFE_SB_PipeId_t PipeId, uint8 *OptsPtr)
{
    ...
    if (OptsPtr == NULL)
    {
        ...
        CFE_EVS_SendEventWithAppID(CFE_SB_GETPIPEOPTS_PTR_ERR_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                                   "Pipe Opts Error:Bad Argument,Requestor %s", CFE_SB_GetAppTskName(TskId, FullName));
        return CFE_SB_BAD_ARGUMENT;
    }
...
int32 CFE_SB_GetPipeName(char *PipeNameBuf, size_t PipeNameSize, CFE_SB_PipeId_t PipeId)
{
    ...
    {
        CFE_ES_GetTaskID(&TskId);
        CFE_EVS_SendEventWithAppID(CFE_SB_GETPIPENAME_NULL_PTR_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
    ...
    {
        CFE_ES_GetTaskID(&TskId);
        CFE_EVS_SendEventWithAppID(CFE_SB_GETPIPENAME_ID_ERR_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                                   "Pipe Id Error:Bad Argument,Id=%d,Requestor %s", PipeId,
                                   CFE_SB_GetAppTskName(TskId, FullName));
    ...
    {
        if (OS_QueueGetInfo(CFE_SB.PipeTbl[PipeId].SysQueueId, &queue_prop) == OS_SUCCESS)
        {
            strncpy(PipeNameBuf, queue_prop.name, PipeNameSize - 1);

            PipeNameBuf[PipeNameSize - 1] = '\0';

            CFE_EVS_SendEventWithAppID(CFE_SB_GETPIPENAME_EID, CFE_EVS_EventType_DEBUG, CFE_SB.AppId,
                                       "GetPipeName name=%s id=%d", PipeNameBuf, PipeId);
        }
        else
        {
            CFE_ES_GetTaskID(&TskId);
...
int32 CFE_SB_GetPipeIdByName(CFE_SB_PipeId_t *PipeIdPtr, const char *PipeName)
{
    ...
    else
    {
        ...
        if (RtnFromVal == OS_SUCCESS)
        {
            ...
            for (PipeTblIdx = 0; PipeTblIdx < CFE_PLATFORM_SB_MAX_PIPES; PipeTblIdx++)
            {
            ...
            if (PipeTblIdx == CFE_PLATFORM_SB_MAX_PIPES)
            {
                ...
                CFE_EVS_SendEventWithAppID(CFE_SB_GETPIPEIDBYNAME_NAME_ERR_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                                           "Pipe ID By Name Error:Bad Argument,Requestor %s",
                                           CFE_SB_GetAppTskName(TskId, FullName));
            ...
            {
                CFE_EVS_SendEventWithAppID(CFE_SB_GETPIPEIDBYNAME_EID, CFE_EVS_EventType_DEBUG, CFE_SB.AppId,
                                           "PipeIdByName name=%s id=%d", PipeName, *PipeIdPtr);
...
{
    return CFE_SB_SubscribeFull(MsgId, PipeId, CFE_SB_Default_Qos, MsgLim, (uint8)CFE_SB_LOCAL);
                           ...
                           uint8 Scope)
{
    ...
    CFE_EVS_SendEventWithAppID(CFE_SB_SUBSCRIPTION_RCVD_EID, CFE_EVS_EventType_DEBUG, CFE_SB.AppId,
                               ...
                               (unsigned int)CFE_SB_MsgIdToValue(MsgId), PipeName, (int)PipeId,
                               CFE_SB_GetAppTskName(TskId, FullName));
...
**
******************************************************************************/
int32 CFE_SB_UnsubscribeWithAppId(CFE_SB_MsgId_t MsgId, CFE_SB_PipeId_t PipeId, CFE_ES_ResourceID_t AppId)
{
    int32 Status = 0;
...
**
******************************************************************************/
int32 CFE_SB_UnsubscribeFull(CFE_SB_MsgId_t MsgId, CFE_SB_PipeId_t PipeId, uint8 Scope, CFE_ES_ResourceID_t AppId)
{
    CFE_SBR_RouteId_t      RouteId;
    uint32                 PipeIdx;
    CFE_ES_ResourceID_t    TskId;
    ...
    /* check input parameters */
    if (!CFE_SB_IsValidMsgId(MsgId) || (CFE_SB_ValidatePipeId(PipeId) != CFE_SUCCESS) || (Scope > 1))
    {
        CFE_SB_UnlockSharedData(__func__, __LINE__);
                                 ...
                                 CFE_SBR_RouteId_t *RouteIdPtr)
{
    ...
    if (!CFE_SB_IsValidMsgId(*MsgIdPtr))
    {
        CFE_EVS_SendEventWithAppID(CFE_SB_SEND_INV_MSGID_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                                   ...
                                   (unsigned int)CFE_SB_MsgIdToValue(*MsgIdPtr), CFE_SB_GetAppTskName(TskId, FullName));
        return CFE_SB_BAD_ARGUMENT;
    } /* end if */
    ...
    if (*SizePtr > CFE_MISSION_SB_MAX_SB_MSG_SIZE)
    {
        CFE_EVS_SendEventWithAppID(CFE_SB_MSG_TOO_BIG_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                                   ...
                                   (unsigned int)CFE_SB_MsgIdToValue(*MsgIdPtr), CFE_SB_GetAppTskName(TskId, FullName),
                                   (int)*SizePtr, CFE_MISSION_SB_MAX_SB_MSG_SIZE);
 ...
 */
int32 CFE_SB_TransmitBufferFull(CFE_SB_BufferD_t *BufDscPtr, CFE_SBR_RouteId_t RouteId, CFE_SB_MsgId_t MsgId)
{
    CFE_ES_ResourceID_t      AppId;
    CFE_ES_ResourceID_t      TskId;
    CFE_SB_DestinationD_t *  DestPtr;
    ...
    for (DestPtr = CFE_SBR_GetDestListHeadPtr(RouteId); DestPtr != NULL; DestPtr = DestPtr->Next)
    {
        ...
        if ((PipeDscPtr->Opts & CFE_SB_PIPEOPTS_IGNOREMINE) != 0 && CFE_ES_ResourceID_Equal(PipeDscPtr->AppId, AppId))
        {
            continue;
        } /* end if */
        ...
        else
        {
            ...
            if (CFE_SB_RequestToSendEvent(TskId, CFE_SB_Q_WR_ERR_EID_BIT) == CFE_SB_GRANTED)
            {
                ...
                CFE_EVS_SendEventWithAppID(
                    CFE_SB_Q_WR_ERR_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                    "Pipe Write Err,MsgId 0x%x,pipe %s,sender %s,stat 0x%x", (unsigned int)CFE_SB_MsgIdToValue(MsgId),
                    PipeName, CFE_SB_GetAppTskName(TskId, FullName), (unsigned int)SBSndErr.EvtBuf[i].ErrStat);
...
int32 CFE_SB_ReceiveBuffer(CFE_SB_Buffer_t **BufPtr, CFE_SB_PipeId_t PipeId, int32 TimeOut)
{
    ...
    if ((BufPtr == NULL) || (TimeOut < (-1)))
    {
        ...
        CFE_EVS_SendEventWithAppID(CFE_SB_RCV_BAD_ARG_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                                   "Rcv Err:Bad Input Arg:BufPtr 0x%lx,pipe %d,t/o %d,app %s", (unsigned long)BufPtr,
                                   (int)PipeId, (int)TimeOut, CFE_SB_GetAppTskName(TskId, FullName));
        return CFE_SB_BAD_ARGUMENT;
    } /* end if */
    ...
    if (PipeDscPtr == NULL)
    {
        ...
        CFE_EVS_SendEventWithAppID(CFE_SB_BAD_PIPEID_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                                   "Rcv Err:PipeId %d does not exist,app %s", (int)PipeId,
                                   CFE_SB_GetAppTskName(TskId, FullName));
        return CFE_SB_BAD_ARGUMENT;
    } /* end if */
...
int32 CFE_SB_ReadQueue(CFE_SB_PipeD_t *PipeDscPtr, CFE_ES_ResourceID_t TskId, CFE_SB_TimeOut_t Time_Out,
                       CFE_SB_BufferD_t **Message)
{
    ...
    Status = OS_QueueGet(PipeDscPtr->SysQueueId, (void *)Message, sizeof(CFE_SB_BufferD_t *), &Nbytes, TimeOut);
    ...
    switch (Status)
    {
        ...
        default:
            ...
            CFE_EVS_SendEventWithAppID(CFE_SB_Q_RD_ERR_EID, CFE_EVS_EventType_ERROR, CFE_SB.AppId,
                                       "Pipe Read Err,pipe %s,app %s,stat 0x%x", PipeName,
                                       CFE_SB_GetAppTskName(TskId, FullName), (unsigned int)Status);
