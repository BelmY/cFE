...
void UT_InitializeTableRegistryNames()
{
    ...
    {
        snprintf(CFE_TBL_TaskData.Registry[i].Name, CFE_TBL_MAX_FULL_NAME_LEN, "%d", i);
...
void Test_CFE_TBL_TaskInit(void)
{
    ...
    UT_Report(__FILE__, __LINE__,
              UT_EventIsInHistory(CFE_TBL_MID_ERR_EID) && CFE_TBL_TaskData.CommandCounter == 0 &&
                  CFE_TBL_TaskData.CommandErrorCounter == 0,
    ...
    UT_Report(__FILE__, __LINE__,
              UT_EventIsInHistory(CFE_TBL_RESET_INF_EID) && CFE_TBL_TaskData.CommandCounter == 0 &&
                  CFE_TBL_TaskData.CommandErrorCounter == 0,
...
void Test_CFE_TBL_DeleteCDSCmd(void)
{
    ...
    {
        snprintf(CFE_TBL_TaskData.CritReg[j - CFE_PLATFORM_TBL_MAX_NUM_TABLES].Name, CFE_TBL_MAX_FULL_NAME_LEN, "%d",
                 j);
    }

    strncpy(DelCDSCmd.Payload.TableName, "-1", sizeof(DelCDSCmd.Payload.TableName));
    UT_Report(__FILE__, __LINE__, CFE_TBL_DeleteCDSCmd(&DelCDSCmd) == CFE_TBL_INC_ERR_CTR, "CFE_TBL_DeleteCDSCmd",
...
void Test_CFE_TBL_AbortLoadCmd(void)
{
    ...
    UT_Report(__FILE__, __LINE__, CFE_TBL_TaskData.LoadBuffs[0].Taken == true, "CFE_TBL_AbortLoad",
...
void Test_CFE_TBL_ActivateCmd(void)
{
    ...
    strncpy(ActivateCmd.Payload.TableName, CFE_TBL_TaskData.Registry[0].Name, sizeof(ActivateCmd.Payload.TableName));
...
void Test_CFE_TBL_ValidateCmd(void)
{
    ...
    for (i = 0; i < CFE_PLATFORM_TBL_MAX_NUM_VALIDATIONS; i++)
    {
        CFE_TBL_TaskData.ValidationResults[i].State = CFE_TBL_VALIDATION_PENDING;
    }
...
void Test_CFE_TBL_GetTblRegData(void)
{
    ...
    UT_Report(__FILE__, __LINE__, CFE_TBL_TaskData.TblRegPacket.Payload.InactiveBufferAddr != 0,
              "CFE_TBL_GetTblRegData", "Double buffered table");
    ...
    CFE_TBL_GetTblRegData();
    UT_Report(__FILE__, __LINE__, CFE_TBL_TaskData.TblRegPacket.Payload.InactiveBufferAddr != 0,
              "CFE_TBL_GetTblRegData", "Single buffered table - inactive buffer");
    ...
    CFE_TBL_GetTblRegData();
    UT_Report(__FILE__, __LINE__, CFE_TBL_TaskData.TblRegPacket.Payload.InactiveBufferAddr == 0,
              "CFE_TBL_GetTblRegData", "No inactive buffer");
}
...
void Test_CFE_TBL_GetHkData(void)
{
    ...
    CFE_TBL_GetHkData();
    UT_Report(__FILE__, __LINE__, CFE_TBL_TaskData.HkPacket.Payload.NumLoadPending == 1, "CFE_TBL_GetHkData",
    ...
    CFE_TBL_GetHkData();
    UT_Report(__FILE__, __LINE__,
              CFE_TBL_TaskData.HkPacket.Payload.NumFreeSharedBufs == CFE_PLATFORM_TBL_MAX_SIMULTANEOUS_LOADS - 1,
    ...
    CFE_TBL_GetHkData();
    UT_Report(__FILE__, __LINE__, CFE_TBL_TaskData.SuccessValCounter == 1, "CFE_TBL_GetHkData",
    ...
    CFE_TBL_GetHkData();
    UT_Report(__FILE__, __LINE__, CFE_TBL_TaskData.FailedValCounter == 1, "CFE_TBL_GetHkData",
              "ValPtr result != CFE_SUCCESS");
    ...
    CFE_TBL_GetHkData();
    UT_Report(__FILE__, __LINE__, CFE_TBL_TaskData.HkPacket.Payload.LastUpdateTime.Seconds == 19283,
              "CFE_TBL_GetHkData", "Invalid registry entry");
    ...
    CFE_TBL_GetHkData();
    UT_Report(__FILE__, __LINE__, CFE_TBL_TaskData.HkPacket.Payload.LastUpdateTime.Seconds == 12345,
              "CFE_TBL_GetHkData", "Last valid table updated out of range (low)");
    ...
    CFE_TBL_GetHkData();
    UT_Report(__FILE__, __LINE__, CFE_TBL_TaskData.HkPacket.Payload.LastUpdateTime.Seconds == 54321,
              "CFE_TBL_GetHkData", "Last valid table updated out of range (high)");
}
...
void Test_CFE_TBL_DumpRegCmd(void)
{
    ...
    /* Test with an error creating the dump file */
    UT_InitData();
    strncpy((char *)DumpRegCmd.Payload.DumpFilename, "", sizeof(DumpRegCmd.Payload.DumpFilename));
    UT_SetDefaultReturnValue(UT_KEY(OS_OpenCreate), OS_ERROR);
    UT_Report(__FILE__, __LINE__, CFE_TBL_DumpRegistryCmd(&DumpRegCmd) == CFE_TBL_INC_ERR_CTR,
              "CFE_TBL_DumpRegistryCmd", "Error creating dump file");

    /* Test with an error writing the cFE File header */
    UT_InitData();
    UT_SetDeferredRetcode(UT_KEY(CFE_FS_WriteHeader), 1, sizeof(CFE_FS_Header_t) - 1);
    UT_Report(__FILE__, __LINE__, CFE_TBL_DumpRegistryCmd(&DumpRegCmd) == CFE_TBL_INC_ERR_CTR,
              "CFE_TBL_DumpRegistryCmd", "Error writing cFE File header");
    ...
    CFE_TBL_TaskData.Registry[0].LoadInProgress = CFE_TBL_NO_LOAD_IN_PROGRESS;
    UT_Report(__FILE__, __LINE__, CFE_TBL_DumpRegistryCmd(&DumpRegCmd) == CFE_TBL_INC_CMD_CTR,
              "CFE_TBL_DumpRegistryCmd", "File did exist previously: successfully overwritten table");
    ...
    UT_Report(__FILE__, __LINE__, CFE_TBL_DumpRegistryCmd(&DumpRegCmd) == CFE_TBL_INC_ERR_CTR,
              "CFE_TBL_DumpRegistryCmd", "Table is not owned, OS_write fails: Error writing Registry");
...
void Test_CFE_TBL_DumpCmd(void)
{
     ...
     */
    UT_InitData();
    strncpy(CFE_TBL_TaskData.Registry[2].Name, "DumpCmdTest", CFE_TBL_MAX_FULL_NAME_LEN);
    CFE_TBL_TaskData.Registry[2].Name[CFE_TBL_MAX_FULL_NAME_LEN - 1] = '\0';
    CFE_TBL_TaskData.Registry[2].OwnerAppId                          = AppID;
    strncpy(DumpCmd.Payload.TableName, CFE_TBL_TaskData.Registry[2].Name, sizeof(DumpCmd.Payload.TableName));
...
void Test_CFE_TBL_LoadCmd(void)
{
    ...
    UT_InitData();
    strncpy((char *)LoadCmd.Payload.LoadFilename, "LoadFileName", sizeof(LoadCmd.Payload.LoadFilename));
    UT_SetDefaultReturnValue(UT_KEY(OS_OpenCreate), OS_ERROR);
    ...
    strncpy((char *)TblFileHeader.TableName, CFE_TBL_TaskData.Registry[0].Name, sizeof(TblFileHeader.TableName));
    strncpy(StdFileHeader.Description, "FS header description", sizeof(StdFileHeader.Description));
    ...
    CFE_TBL_TaskData.Registry[0].Size = sizeof(CFE_TBL_File_Hdr_t);
    strncpy(TblFileHeader.TableName, CFE_TBL_TaskData.Registry[0].Name, sizeof(TblFileHeader.TableName));
    UT_SetReadBuffer(&TblFileHeader, sizeof(TblFileHeader));
    UT_SetReadHeader(&StdFileHeader, sizeof(StdFileHeader));
    UT_Report(__FILE__, __LINE__, CFE_TBL_LoadCmd(&LoadCmd) == CFE_TBL_INC_ERR_CTR, "CFE_TBL_LoadCmd",
              "File has partial load for uninitialized table and offset "
    ...
    UT_InitData();
    strncpy(LoadCmd.Payload.LoadFilename, "LoadFileName", sizeof(LoadCmd.Payload.LoadFilename));
...
void Test_CFE_TBL_HousekeepingCmd(void)
{
     ...
     */
    UT_InitData();
    strncpy(CFE_TBL_TaskData.DumpControlBlocks[0].TableName, "housekeepingtest", CFE_TBL_MAX_FULL_NAME_LEN);
...
void Test_CFE_TBL_Share(void)
{
    ...
    StdFileHeader.ContentType = CFE_FS_FILE_CONTENT_ID;
    StdFileHeader.SubType     = CFE_FS_SubType_TBL_IMG;
    strncpy((char *)TblFileHeader.TableName, "ut_cfe_tbl.UT_Table4", sizeof(TblFileHeader.TableName));
...
void Test_CFE_TBL_Load(void)
{
    ...
    UT_InitData();
    strncpy(StdFileHeader.Description, "Test description", sizeof(StdFileHeader.Description));
    StdFileHeader.ContentType = CFE_FS_FILE_CONTENT_ID;
    StdFileHeader.SubType     = CFE_FS_SubType_TBL_IMG;
    strncpy((char *)TblFileHeader.TableName, "ut_cfe_tbl.UT_Table1", sizeof(TblFileHeader.TableName));
    ...
    StdFileHeader.ContentType = CFE_FS_FILE_CONTENT_ID;
    StdFileHeader.SubType     = CFE_FS_SubType_TBL_IMG;
    strncpy((char *)TblFileHeader.TableName, "ut_cfe_tbl.NotUT_Table1", sizeof(TblFileHeader.TableName));
    UT_TBL_SetupHeader(&TblFileHeader, 0, sizeof(UT_Table1_t));
    ...
    StdFileHeader.ContentType = CFE_FS_FILE_CONTENT_ID;
    StdFileHeader.SubType     = CFE_FS_SubType_TBL_IMG;
    strncpy(TblFileHeader.TableName, "ut_cfe_tbl.NotUT_Table1", sizeof(TblFileHeader.TableName));
    UT_TBL_SetupHeader(&TblFileHeader, 0, sizeof(UT_Table1_t));
    ...
    EventsCorrect              = (UT_EventIsInHistory(CFE_SUCCESS) == true && UT_GetNumEventsSent() == 1);
    UT_Report(__FILE__, __LINE__, RtnCode == CFE_TBL_ERR_FILE_FOR_WRONG_TABLE && EventsCorrect, "CFE_TBL_Load",
    ...
    UT_SetDeferredRetcode(UT_KEY(Test_CFE_TBL_ValidationFunc), 1, 1);
    RtnCode       = CFE_TBL_Load(App1TblHandle1, CFE_TBL_SRC_ADDRESS, &TestTable1);
    EventsCorrect = (UT_EventIsInHistory(CFE_SUCCESS) == true && UT_GetNumEventsSent() == 2);
    UT_Report(__FILE__, __LINE__, RtnCode == -1 && EventsCorrect, "CFE_TBL_Load",
...
void Test_CFE_TBL_Manage(void)
{
    ...
    UT_Report(__FILE__, __LINE__,
              RtnCode == CFE_SUCCESS && EventsCorrect && CFE_TBL_TaskData.ValidationResults[0].Result == -1,
              "CFE_TBL_Manage",
    ...
    UT_Report(__FILE__, __LINE__,
              RtnCode == CFE_SUCCESS && EventsCorrect && CFE_TBL_TaskData.ValidationResults[0].Result == 0,
              "CFE_TBL_Manage",
    ...
    strncpy(CFE_TBL_TaskData.DumpControlBlocks[0].DumpBufferPtr->DataSource, "MyDumpFilename", OS_MAX_PATH_LEN - 1);
    CFE_TBL_TaskData.DumpControlBlocks[0].DumpBufferPtr->DataSource[OS_MAX_PATH_LEN - 1] = 0;
    strncpy(CFE_TBL_TaskData.DumpControlBlocks[0].TableName, "ut_cfe_tbl.UT_Table2", CFE_TBL_MAX_FULL_NAME_LEN - 1);
    CFE_TBL_TaskData.DumpControlBlocks[0].TableName[CFE_TBL_MAX_FULL_NAME_LEN - 1] = 0;
...
void Test_CFE_TBL_TblMod(void)
{
    ...
    strncpy(FileHeader.Description, "FS header description", sizeof(FileHeader.Description));
    ...
    strncpy((char *)File.TblHeader.TableName, "ut_cfe_tbl.UT_Table1", sizeof(File.TblHeader.TableName));
    ...
    FileHeader.TimeSeconds    = 1704;
    FileHeader.TimeSubSeconds = 104;
    strncpy((char *)File.TblHeader.TableName, "ut_cfe_tbl.UT_Table2", sizeof(File.TblHeader.TableName));
    UT_TBL_SetupHeader(&File.TblHeader, 0, sizeof(UT_Table1_t));
    ...
    UT_Report(__FILE__, __LINE__,
              RtnCode == CFE_SUCCESS && RtnCode2 == CFE_SUCCESS && EventsCorrect &&
                  TblInfo1.TimeOfLastUpdate.Seconds == TblInfo1.TimeOfLastUpdate.Subseconds &&
                  strncmp(TblInfo1.LastFileLoaded, MyFilename, OS_MAX_PATH_LEN - 4) == 0 &&
...
void Test_CFE_TBL_Internal(void)
{
    ...
    AccessDescPtr = &CFE_TBL_TaskData.Handles[App1TblHandle2];
    RegRecPtr     = &CFE_TBL_TaskData.Registry[AccessDescPtr->RegIndex];
    strncpy(RegRecPtr->Name, "ut_cfe_tbl.UT_Table3", CFE_TBL_MAX_FULL_NAME_LEN);
    ...
    StdFileHeader.ContentType                              = CFE_FS_FILE_CONTENT_ID;
    StdFileHeader.SubType                                  = CFE_FS_SubType_TBL_IMG;
    strncpy((char *)TblFileHeader.TableName, "ut_cfe_tbl.UT_Table2", sizeof(TblFileHeader.TableName));
    ...
    StdFileHeader.ContentType     = CFE_FS_FILE_CONTENT_ID;
    StdFileHeader.SubType         = CFE_FS_SubType_TBL_IMG;
    strncpy((char *)TblFileHeader.TableName, "ut_cfe_tbl.NotUT_Table2", sizeof(TblFileHeader.TableName));
    UT_TBL_SetupHeader(&TblFileHeader, 0, sizeof(UT_Table1_t));
    ...
    StdFileHeader.ProcessorID     = CFE_PLATFORM_TBL_VALID_PRID_1;
    strncpy(TblFileHeader.TableName, "ut_cfe_tbl.UT_Table1", sizeof(TblFileHeader.TableName));
    TblFileHeader.NumBytes = sizeof(UT_Table1_t) - 1;
    TblFileHeader.Offset   = 0;
